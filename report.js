...
function populateField(elementId, storageKey, options = {}) {
  ...
  const defaultValue =
    options.defaultValue !== undefined ? options.defaultValue : "N/A";
...
function buildBmiChart(elementId, hrFn, baseColor, savedBmi) {
  ...
  if (!element) {
    console.error(
      `BMI Chart Error: Canvas element with ID "${elementId}" not found.`
    );
    return null;
  }
  const colorMap = {
    crimson: "rgba(220,20,60,0.2)",
    royalblue: "rgba(65,105,225,0.2)",
    seagreen: "rgba(46,139,87,0.2)",
  };
  const lineData = xs_bmi.map((x) => ({
    x: x,
    y:
      savedBmi && Math.abs(x - savedBmi) < GAP_BMI ? null : (hrFn(x) - 1) * 100,
  }));
...
function updateBmiCharts(charts, bmiValue) {
  ...
  charts.forEach(({ chart, fn }) => {
    ...
    chart.data.datasets = chart.data.datasets.filter(
      (ds) => ds.label !== "User BMI"
    ); // Remove old scatter if exists
    chart.data.datasets[0].data = xs_bmi.map((x) => ({
      x: x,
      y:
        isValidBmi && Math.abs(x - bmiValue) < GAP_BMI
          ? null
          : (fn(x) - 1) * 100,
    })); // Update line data for gaps
    ...
    if (isValidBmi) {
      ...
      chart.data.datasets.push({
        ...
        showLine: false,
        order: 99, // Ensure it's drawn on top
      });
    }
...
function hrFMI(fmi) {
  if (fmi < 5) {
    return Math.exp(0.12 * (5 - fmi));
  } else {
    return 1 + 0.02 * Math.pow(fmi - 5, 1.8);
  }
}
function hrFFMI(ffmi) {
  if (ffmi < 22) {
    return Math.exp(0.08 * (22 - ffmi));
  } else {
    return 1 + 0.02 * Math.pow(ffmi - 22, 1.5);
  }
}
...
function buildFmiffmiChart(element, data, label, color, xMin, xMax) {
  ...
  const refLineData =
    xMin === 0
      ? Array.from({ length: 201 }, (_, i) => ({ x: i * 0.1, y: 1 }))
      : Array.from({ length: 161 }, (_, i) => ({ x: 12 + i * 0.1, y: 1 }));
    ...
    options: {
      ...
      scales: {
        x: {
          ...
          title: {
            display: true,
            text: label === "FMI HR" ? "FMI (kg/m²)" : "FFMI (kg/m²)",
          },
          ticks: { stepSize: 2 },
        },
        y: {
          min: 0.5,
          max: 3.5,
          title: { display: true, text: "Hazard ratio" },
          ticks: { stepSize: 0.5 },
        },
      },
      plugins: {
        ...
        tooltip: {
          ...
          callbacks: {
            label: (ctx) =>
              ` ${
                label.includes("FMI") ? "FMI" : "FFMI"
              }: ${ctx.parsed.x.toFixed(1)}, HR: ${ctx.parsed.y.toFixed(2)}`,
          },
        },
      },
    },
...
// --- BP / RHR Charting ---
function rrRHR_J(rhr) {
  if (rhr < 50) return Math.pow(1.12, (50 - rhr) / 10);
  if (rhr <= 60) return 1;
  return Math.pow(1.09, (rhr - 60) / 10);
}
const sbpEquation = (sbp) => {
  const base = Math.pow(1.02, (sbp - 120) / 10);
  return sbp > 140 ? base * (1 + 0.008 * Math.pow(sbp - 140, 1.3)) : base;
};
const dbpEquation = (dbp) => {
  const base = Math.pow(1.02, (dbp - 80) / 5);
  return dbp > 90 ? base * (1 + 0.01 * Math.pow(dbp - 90, 1.2)) : base;
};
const genData = (range, step, fn) => {
  let d = [];
  for (let x = range[0]; x <= range[1]; x += step) {
    d.push({ x: x, y: parseFloat(fn(x).toFixed(3)) });
  }
  return d;
};

const createRiskCurveChart = (
  ctx,
  label,
  dataPoints,
  color,
  ...
  if (!ctx) return null;
  const eqMap = {
    "Resting Heart Rate (bpm)": rrRHR_J,
    "Systolic BP (mmHg)": sbpEquation,
    "Diastolic BP (mmHg)": dbpEquation,
  };
  ...
  return new Chart(ctx, {
    ...
    data: {
      datasets: [
        ...
        {
          label: "Reference HR=1.0",
          data: dataPoints.map((p) => ({ x: p.x, y: 1 })),
          borderColor: "#666",
          borderDash: [6, 4],
          pointRadius: 0,
          borderWidth: 1,
    ...
    options: {
      ...
      scales: {
        y: {
          ...
          max: yMax,
          ticks: {
            callback: (v) => v.toFixed(yType === "logarithmic" ? 1 : 2),
          },
        },
        x: {
          type: "linear",
      ...
      plugins: {
        ...
        tooltip: {
          callbacks: {
            label: (tCtx) =>
              ` ${label}: ${tCtx.parsed.x.toFixed(
                0
              )}, Risk: ${tCtx.parsed.y.toFixed(2)}x`,
          },
        },
      },
    },
...
function coeffs(sex, age) {
  ...
  return (
    RANTANEN[validSex].find((r) => ageNum <= r.max) ||
    RANTANEN[validSex][RANTANEN[validSex].length - 1]
  );
}
...
function updateRMRComparisonVisualization() {
  ...
  const predictedRMR = parseFloat(
    String(predictedRMRStr).replace(/[^0-9.]/g, "")
  );
  const measuredRMR = parseFloat(
    String(measuredRMRStr).replace(/[^0-9.]/g, "")
  );

  // Check if both values are valid numbers
  if (isNaN(predictedRMR) || isNaN(measuredRMR) || predictedRMR === 0) {
    console.warn("Invalid RMR values for comparison visualization:", {
      predictedRMR,
      measuredRMR,
    });
  ...
  if (percentDiffEl) {
    percentDiffEl.textContent = `Measured RMR is ${Math.abs(diff).toFixed(
      1
    )}% ${diff >= 0 ? "higher" : "lower"} than predicted`;
  }
...
document.addEventListener("DOMContentLoaded", () => {
  console.log(
    "DOM fully loaded and parsed. Initializing report data and charts..."
  );
  ...
  if (gender && !isNaN(ageNum) && !isNaN(gripAvgValue)) {
    ...
    console.warn(
      "Cannot calculate Grip Note due to missing/invalid gender, age, or grip value."
    );
    gripNote = "N/A";
  }
  ...
  if (!isNaN(peakHr) && !isNaN(hr1minRec)) {
    ...
    console.warn(
      "Cannot calculate HRR due to missing/invalid peak HR or 1-min recovery HR."
    );
  ...
  if (summaryRmrElement) {
    const rmrValue =
      measuredRMRStr && String(measuredRMRStr).trim() !== ""
        ? String(measuredRMRStr)
            .replace(/ kcal\/day/gi, "")
            .trim()
        : predictedRMRStr
        ? String(predictedRMRStr)
            .replace(/ kcal\/day/gi, "")
            .trim()
        : "N/A";
  ...
  if (!isNaN(measuredRMR) && !isNaN(predictedRMR) && predictedRMR !== 0) {
    const percentDifference =
      ((measuredRMR - predictedRMR) / predictedRMR) * 100;
    rmrNote = getRmrNote(percentDifference);
    console.log("RMR Calculation:", {
      measuredRMR,
      predictedRMR,
      percentDifference,
      rmrNote,
    ...
    console.warn(
      "Cannot calculate RMR Note due to missing/invalid measured or predicted RMR, or predicted RMR is zero."
    );
    // Keep rmrNote as 'N/A'
  }
  ...
  const vo2ValueClean = unifiedVO2Max
    ? String(unifiedVO2Max)
        .replace(/ ml\/kg\/min/gi, "")
        .trim()
    : "";
  ...
  if (vo2ValueClean && !isNaN(vo2ValueNum) && vo2ValueNum > 0) {
    ...
    if (gender && !isNaN(ageNum)) {
      ...
      console.warn(
        "Cannot calculate VO2 percentile/note due to missing/invalid gender or age."
      );
    }
    ...
    if (!unifiedVO2Max)
      console.warn(
        "VO2 Max data ('unified_vo2max') not found in localStorage."
      );
  ...
  const rmrGraphValue =
    measuredRMR && String(measuredRMR).trim() !== ""
      ? measuredRMR
      : predictedRMR;
  const graphRmrMeasuredElement = document.getElementById("graphRmrMeasured");
  if (graphRmrMeasuredElement) {
    graphRmrMeasuredElement.value = rmrGraphValue
      ? String(rmrGraphValue)
          .replace(/ kcal\/day/gi, "")
          .trim()
      : "N/A";
  }
  ...
  const repMaxTable = document.querySelector(
    ".page:nth-of-type(11) .table-wrapper table"
  ...
  if (repMaxTable) {
    ...
    for (let i = 1; i <= 15; i++) {
      ...
      if (row && row.cells.length >= 4) {
        ...
        console.warn(
          `Rep Max Table: Row ${i} does not have enough cells or is missing.`
        );
      }
    }
  ...
  if (mortChartAllEl && mortChartCommEl && mortChartNcdEl) {
    ...
    console.warn(
      "One or more BMI chart canvas elements not found. BMI charts not initialized."
    );
  }
  ...
  if (fmiChartElement && ffmiChartElement) {
    ...
    console.warn(
      "FMI or FFMI chart canvas element not found. Charts not initialized."
    );
  }
  ...
  if (sbpChartEl)
    createRiskCurveChart(
      sbpChartEl.getContext("2d"),
      "Systolic BP (mmHg)",
      sbpData,
      "#d9534f",
      userSBP,
  ...
  else console.warn("SBP chart canvas not found.");
  if (dbpChartEl)
    createRiskCurveChart(
      dbpChartEl.getContext("2d"),
      "Diastolic BP (mmHg)",
      dbpData,
      "#5cb85c",
      userDBP,
  ...
  else console.warn("DBP chart canvas not found.");
  if (rhrChartEl)
    createRiskCurveChart(
      rhrChartEl.getContext("2d"),
  ...
  else console.warn("RHR chart canvas not found.");
  ...
  const gripContainer = gripChartEl
    ? gripChartEl.closest(".graph-container")
    : null; // Find the container
  ...
  if (gripChartEl && gripContainer) {
    if (isAgeValid) {
      ...
      const gripSex = (localStorage.getItem("gender") || "MALE")
        .toUpperCase()
        .startsWith("F")
        ? "F"
        : "M";
      ...
      const gripCurve = xs_grip.map((x) => ({
        x: x,
        y: rrGrip(x, gripSex, gripAge),
      }));
      ...
      if (!isNaN(userGripAvg)) {
        ...
        if (!isNaN(userRisk)) {
          // Check if risk calculation was successful
          ...
          console.warn(
            `Could not calculate risk for user grip average: ${userGripAvg}`
          );
        }
      }
      ...
      new Chart(gripChartEl, {
        ...
        data: {
          datasets: [
            ...
            {
              label: "Reference RR=1.0",
              data: xs_grip.map((x) => ({ x: x, y: 1 })),
              borderColor: "#666",
              borderDash: [6, 4],
              pointRadius: 0,
            },
        ...
        options: {
          ...
          scales: {
            x: {
              ...
              min: 10,
              max: 65,
              title: { display: true, text: "Grip strength (kg)" },
            },
            ...
            y: {
              ...
              title: {
                display: true,
                text: "Relative risk (All-Cause Mortality)",
              },
            },
          },
        },
      ...
      console.warn(
        "Grip Strength chart not generated: Age data missing or invalid."
      );
      gripChartEl.style.display = "none"; // Hide the canvas element
      const messageElement = document.createElement("p");
      messageElement.textContent =
        "Age data is missing or invalid. Grip strength chart requires age to be calculated.";
    ...
    if (!gripChartEl)
      console.warn("Grip Strength chart canvas element 'gripChart' not found.");
    if (!gripContainer)
      console.warn(
        "Could not find '.graph-container' for Grip Strength chart."
      );
  }
  ...
  if (strengthChartEl) {
    ...
    if (!annotationPlugin) {
      console.warn(
        "Chart.js Annotation plugin not found or not registered. Score line will not be displayed."
      );
    } else if (!isNaN(totalStrengthScore)) {
      console.log(
        `Adding annotation line for FSS score: ${totalStrengthScore}`
      );
      annotationOptions = {
      ...
      console.warn(
        "Total strength score not found or invalid. Annotation line not added."
      );
    }
      ...
      options: {
        ...
        scales: {
          x: {
            ...
            title: {
              display: true,
              text: "Functional Strength Score",
              font: { size: 14 },
            },
            ticks: { stepSize: 100 },
          },
    ...
    console.warn(
      "Functional Strength chart canvas element 'strengthChart' not found."
    );
  }
  ...
  if (vo2ChartElement) {
    ...
    if (ctx && gender && !isNaN(ageNum) && !isNaN(vo2MetsNum)) {
      console.log(
        `Initializing VO2 Max chart for ${gender}, age ${ageNum}, VO2 METs ${vo2MetsNum.toFixed(
          1
        )}`
      );
      ...
      if (ctx) {
        ...
        ctx.fillText(
          "VO₂ Max chart requires gender, age, and VO₂ data.",
  ...
  if (pdfButton) {
    ...
    console.warn(
      "PDF Generation button not found. PDF generation must be triggered manually or via existing onclick."
    );
  }
